...
def _init_fn(worker_id):
    np.random.seed()
    random.seed()
torch.manual_seed(seed)  # set again
...
scale_factor = maxdisp / 384. # controls training resolution

# all_left_img, all_right_img, all_left_disp, all_right_disp = ls.dataloader('%s/carla-highres/trainingF'%database)
...
# all_left_img, all_right_img, all_left_disp, all_right_disp = lt.dataloader('%s/sceneflow/'%database)
...
all_left_img, all_right_img, all_left_disp = lk12.dataloader('%s/stereo2012/data_stereo_flow/training/'%database)
loader_kitti12 = DA.myImageFloder(all_left_img,all_right_img,all_left_disp, rand_scale=[0.9,2.4*scale_factor], order=0)

# all_left_img, all_right_img, all_left_disp, _ = ls.dataloader('%s/eth3d/'%database)
...
data_inuse = torch.utils.data.ConcatDataset( [loader_mb]*500  + [loader_kitti15] + [loader_kitti12]*80 )
...
log = logger.Logger(savemodel, name=logname)
total_iters = 0
...
for epoch in range(1, epochs+1):
    ...
    for batch_idx, (imgL_crop, imgR_crop, disp_crop_L) in enumerate(TrainImgLoader):
        ...
        if (total_iters + 1)%2000==0:
            #SAVE
            savefilename = savemodel+'/'+logname+'/finetune_'+str(total_iters)+'.tar'
            torch.save({
                'iters': total_iters,
