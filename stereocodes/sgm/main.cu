...
int main(int argc, char *argv[]) {
	...
	const char* disparity_dir = "predictions_sgm";
	...
	while ((ep = readdir(dp)) != NULL) {
		...
		cv::Mat h_im0;
		cv::Mat h_im0o = cv::imread(left_file);
		cv::Mat h_im1;
		cv::Mat h_im1o = cv::imread(right_file);
		h_im0=h_im0o;
		h_im1=h_im1o;
		...
		float fx0 = std::ceil(h_im0o.cols/4.0)*4/h_im0o.cols;
		float fy0 = std::ceil(h_im0o.rows/4.0)*4/h_im0o.rows;
		cv::resize(h_im0o, h_im0, cv::Size(0, 0), fx0, fy0);

		float fx1 = std::ceil(h_im1.cols/4.0)*4/h_im1o.cols;
		float fy1 = std::ceil(h_im1o.rows/4.0)*4/h_im1o.rows;
		cv::resize(h_im1o, h_im1, cv::Size(0, 0), fx1, fy1);
...
#endif
std::cout<<"cnt = "<<cnt<<"   file = "<<left_file <<std::endl;
if(cnt>20){
	cnt=0;
